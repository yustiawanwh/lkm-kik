...
import { rekapKelas, rekapKeCSV, LABEL_RANAH } from '../lib/data-rekap.js';
...
export async function renderRekap(root, { profil, onKeluar }) {
  ...
  async function muatKelas() {
    ...
    try {
      ...
      if (kelasList.length > 0) { kelasTerpilih = kelasList[0].id; await muatRekap(); }
  ...
  async function muatRekap() {
    ...
    memuat = true; render();
    try {
      const hasil = await rekapKelas(kelasTerpilih);
      ...
      konten: [
        el('div', { style: 'display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;flex-wrap:wrap;gap:8px;' }, [
          el('select', {
            ...
            onchange: (e) => { kelasTerpilih = e.target.value; muatRekap(); }
